...
export class HeldenService {
  ...
  loadHeld(xml : string, callback : (held:Held) => void):void {
    ...
    const ausruestung = this.extractAusruestung(xmlDoc, attribute[6].value);
  ...
  private extractAusruestung(xmlDoc: Document, kk: number): Ausruestung {
      ...
      if(type.startsWith('nkwaffe')) {
        ...
        this.ausruetungsService.getWaffeByName(name).subscribe(
          (waffe: Waffe) => {
            ...
            waffe.totalSchaden = {w6:waffe.schaden.w6, fix: waffe.schaden.fix}
