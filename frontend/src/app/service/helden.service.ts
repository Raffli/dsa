...
export class HeldenService {
  ...
  private extractAusruestung(xmlDoc: Document, kk: number, kampftalente: KampfTalent[], atBasis: number, paBasis: number, fkBasis: number,  sonderfertigkeiten: Sonderfertigkeit[]): Ausruestung {
      ...
      if (type.startsWith('nkwaffe')) {
          ...
          this.ausruetungsService.getSchildByName(name).subscribe(
            (data: Schild) => {
              ...
              if (this.hasSonderfertigkeit('Linkhand', sonderfertigkeiten)) {
                bonusPa++;
                if (this.hasSonderfertigkeit('Schildkampf I', sonderfertigkeiten)) {
                  bonusPa += 2;
                }
                console.log('TODO check schildkampf names')
                if (this.hasSonderfertigkeit('Schildkampf II', sonderfertigkeiten)) {
                  bonusPa += 2;
                }
              }
        ...
        this.ausruetungsService.getRuestungByName(name).subscribe(
          (data: Ruestung) => {
            ...
            if (this.hasSonderfertigkeit('Rüstungsgewöhnung I', sonderfertigkeiten, data.name)) {
  ...
  private hasSonderfertigkeit(name: string, sonderfertigkeiten: Sonderfertigkeit[], gegenstandName?: string ): boolean {
    for (let i = 0; i < sonderfertigkeiten.length; i++) {
      if (sonderfertigkeiten[i].name === name) {
        if (gegenstandName !== undefined) {
          console.log(sonderfertigkeiten[i])
          if (gegenstandName === sonderfertigkeiten[i].info) {
            return true;
          }
        } else {
          return true;
