    ...
    this.ausruetungsService.getEquipmentByNameAndType(ausruestungBatch).subscribe(
      (data: any[]) => {
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (data[i] === null) {
            console.log('Unable to extract equipment with name: ' + ausruestungBatch[i].name);
            continue;
          }
