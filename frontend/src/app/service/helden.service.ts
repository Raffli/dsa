...
export class HeldenService {
  ...
  loadHeld(xml : string, callback : (held:Held) => void):void {
    ...
    this.extractTalente(xmlDoc, attribute[17].value, (talente: Talente) => {
      ...
      const sonderfertigkeiten = this.extractSonderfertigkeiten(xmlDoc, talente);
      const ausruestung = this.extractAusruestung(xmlDoc, attribute[7].value, talente.kampftalente, attribute[15].value, attribute[16].value, sonderfertigkeiten);
  ...
  private extractAusruestung(xmlDoc: Document, kk: number, kampftalente: KampfTalent[], atBasis: number, paBasis: number, sonderfertigkeiten: Sonderfertigkeit[]): Ausruestung {
  ...
  private extractSonderfertigkeiten(xmlDoc: Document, talente: Talente) : Sonderfertigkeit[] {
    ...
    for (let i = 0; i < nodes.length ; i++) {
      ...
      let info = null;
      let spezialisierung = null;
      if (node.childNodes.length != 0) {

        let child = node.childNodes[0];
        info = child.attributes.getNamedItem('name').value
        if(node.childNodes.length > 1) {
          spezialisierung = node.childNodes[1].attributes.getNamedItem('name').value;
          if (child.nodeName === 'talent') {
            let talentName = name.substr(22);
            talentName = talentName.substr(0, talentName.indexOf(' '))
            for (let i = 0; i<talente.kampftalente.length; i++) {
              if (talente.kampftalente[i].name === talentName) {

                const sf = new Sonderfertigkeit(name,info,spezialisierung);
                talente.kampftalente[i].attachSf(sf)
                sonderfertigkeiten.push(sf);
              }
            }
          }
        ...
        //TODO: handle spezialisierung
      }
      const sonderfertigkeit = new Sonderfertigkeit(name, info, spezialisierung);
